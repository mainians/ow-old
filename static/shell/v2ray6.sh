...
tls_config() {
	...
	echo
	echo
	echo -e "$yellow 请将 $magenta$domain$none $yellow AAAA解析到: $cyan$ip$none"
	echo
	echo -e "$yellow 请将 $magenta$domain$none $yellow AAAA解析到: $cyan$ip$none"
	echo
	echo -e "$yellow 请将 $magenta$domain$none $yellow AAAA解析到: $cyan$ip$none"
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		...
		pushd /tmp
		git clone https://scaleya@bitbucket.org/scaleya/v2ray-1.git -b "$_gitbranch" /etc/v2ray/233boy/v2ray --depth=1
		popd
