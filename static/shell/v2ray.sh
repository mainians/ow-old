...
downloadV2Ray(){
    ...
    curl -k  ${PROXY} -L -H "Cache-Control: no-cache" -o ${ZIPFILE} ${DOWNLOAD_LINK}
    if [ $? != 0 ];then
...
getVersion(){
    if [[ -n "$VERSION" ]]; then
        ...
        #edit by Scaleya line 267-275
        #TAG_URL="https://api.github.com/repos/v2ray/v2ray-core/releases/latest"
        TAG_URL="https://data.jsdelivr.com/v1/package/gh/v2ray/v2ray-core"


        #curl -s https://data.jsdelivr.com/v1/package/gh/v2ray/v2ray-core --connect-timeout 10| head -n4 | tail -1 |cut -d\" -f2


        #NEW_VER="$(normalizeVersion "$(curl ${PROXY} -s "${TAG_URL}" --connect-timeout 10| grep 'tag_name' | cut -d\" -f4)")"
        NEW_VER="$(normalizeVersion "$(curl ${PROXY} -sk "${TAG_URL}" --connect-timeout 10| head -n4 | tail -1 |cut -d\" -f2)")"
